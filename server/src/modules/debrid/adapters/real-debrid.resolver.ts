...
import type { IDebridResolver, ResolvedStream, DebridAccountStatus } from '../ports/debrid.ports.js';
...
export class RealDebridResolver implements IDebridResolver {
    ...
    canResolve(stream: Stream): boolean {
        ...
        return !!(stream.infoHash || (stream.url && !stream.url.startsWith('magnet:')));
    }

    async resolve(stream: Stream, requestId: string): Promise<ResolvedStream | null> {
        if (!this.apiToken) {
            logger.debug({ requestId }, 'Real-Debrid not configured, skipping resolution');
            return null;
        }
        ...
        logger.info({ requestId, stream: stream.title }, 'Real-Debrid resolution placeholder');
