...
describe("RealDebridResolver", () => {
  ...
  it("should resolve a cached torrent successfully", async () => {
    ...
    const result = await resolver.resolve(
      { infoHash: "hash123", title: "Test" } as any,
      "req-1",
    );
  ...
  it("should handle file selection for a pending torrent", async () => {
    ...
    const selectCall = fetchMock.mock.calls.find(
      (call) => call[1].method === "POST" && call[0].includes("selectFiles"),
    );
  ...
  it("should return null and delete if torrent fails to cache after polling", async () => {
    ...
    for (let i = 0; i < 6; i++) {
      fetchMock.mockResolvedValueOnce({
        ok: true,
        json: () =>
          Promise.resolve({ id: "torrent-123", status: "downloading" }),
      });
    }
    ...
    expect(fetchMock).toHaveBeenCalledWith(
      expect.stringContaining("/torrents/delete/torrent-123"),
