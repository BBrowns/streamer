...
import {
  ...
  sanitizeMeta,
  sanitizeStreamUrl,
} from "../src/modules/aggregator/sanitizer.js";
...
describe("stripHtml", () => {
  ...
  it("should strip script blocks", () => {
    ...
    expect(
      stripHtml(
        'before<script type="text/javascript">malicious()</script>after',
      ),
    ).toBe("beforeafter");
  });
...
describe("sanitizeStreamUrl", () => {
  ...
  it("should accept magnet links", () => {
    expect(sanitizeStreamUrl("magnet:?xt=urn:btih:abc123")).toBe(
