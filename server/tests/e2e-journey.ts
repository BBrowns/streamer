...
async function runJourney() {
    ...
    try {
        ...
        await axios.post(`${API_URL}/addons`,
        ...
        if (catalogRes.status !== 200 || !catalogRes.data.metas || catalogRes.data.metas.length === 0) {
            throw new Error('Catalog fetch failed or returned empty');
        }

        const firstMovie = catalogRes.data.metas[0];
        console.log(`✅ Catalog fetched successfully. Picked movie: ${firstMovie.name} (ID: ${firstMovie.id})`);
        ...
        console.log('\n5. Fetching Streams...');
        const streamRes = await axios.get(`${API_URL}/stream/movie/${firstMovie.id}`, {
            headers: { Authorization: `Bearer ${accessToken}` }
        });
        ...
        console.log(`✅ Stream request successful. Found ${streamRes.data.streams?.length || 0} streams.`);
