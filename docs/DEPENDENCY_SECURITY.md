...
| Expo/Metro -> `image-size`                                       | Mobile development/build tooling; high parser resource-exhaustion findings | `image-size` has no patched npm release beyond 2.0.2 yet. The exact root node is reviewed because Metro owns the parser and the repository does not pass remote image input to it. Exception expires 2026-09-30 or before the next RC; upgrade Expo/Metro when a fixed release ships.                                                                                                                                                                        |
