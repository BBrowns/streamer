...
export async function destroyClient(): Promise<void> {
    ...
    return new Promise<void>((resolve) => {
        client.destroy((err: Error | null) => {
            if (err) console.error('[stream-server] Error destroying client:', err.message);
            client = null;
            resolve();
        });
    });
...
function handleShutdown(signal: string) {
    console.log(`[stream-server] Received ${signal}, shutting down gracefully...`);
    destroyClient().then(() => {
...
export async function streamRequest(req: Request, res: Response) {
    ...
    try {
        ...
        console.error('[stream-server]', isTimeout ? 'Torrent metadata timeout (no peers found)' : `Torrent error: ${msg}`);
        if (!res.headersSent) {
    ...
    try {
        ...
        console.log(`[stream-server] Redirecting to webtorrent server: ${streamUrl}`);
        return res.redirect(302, streamUrl);
    } catch (err: any) {
        console.error('[stream-server] Failed to start torrent HTTP server:', err?.message);
