...
**Bridge auth and operations:** Control routes use `requireBridgeAuth` when `STREAMER_BRIDGE_TOKEN` is configured. Clients can send either bearer auth or `x-streamer-bridge-token`. The v1 renderer access-session flow is loopback-only and master-authenticated: Electron can revoke a session through `DELETE /api/bridge/v1/access-sessions/:sessionId`, while repeated revocation is intentionally an idempotent success that does not reveal whether a session existed. `GET /api/bridge/v1/metrics` is also loopback/master-only and returns bounded process-local counters for rate limits, session issuance/revocation, idempotency conflicts, and terminal job observations; it never includes ids, source identities, URLs, magnets, hashes, or credentials. The mobile `BridgeClient.getOperationalMetrics()` keeps its own privacy-safe `session_renewed` counter for successful renderer refreshes. Gateway stream URLs stay header-free for native video/cast consumption, but are HMAC-signed with expiry query params before the bridge serves bytes. `BridgeClient` renews an expired renderer session at most once per request, single-flights concurrent renewals, retries GET/job/cast-play requests with the original request body, and deliberately does not retry cast controls automatically.
