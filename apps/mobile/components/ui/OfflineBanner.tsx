...
export function OfflineBanner() {
    ...
    useEffect(() => {
        ...
        if (Platform.OS === 'web') {
            ...
            (import('expo-network') as Promise<any>).then((Network: any) => {
                if (!mounted) return;

                // Check initial state
                Network.getNetworkStateAsync().then((state: { isInternetReachable: boolean | null }) => {
                    if (mounted) setIsOffline(!state.isInternetReachable);
                });

                // Subscribe to changes
                const sub = Network.addNetworkStateListener((state: { isInternetReachable: boolean | null }) => {
                    if (mounted) setIsOffline(!state.isInternetReachable);
                });
