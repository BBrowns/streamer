...
describe("diagnosticsFromDesktopBridge", () => {
  ...
  it("keeps torrent engine architecture failures as unsupported", () => {
    const legacyInfo = {
      ...
      Parameters<typeof diagnosticsFromDesktopBridge>[0]
    >;

    expect(diagnosticsFromDesktopBridge(legacyInfo)).toMatchObject({
      status: "unsupported",
      reason: "native-architecture-mismatch",
      processArch: "x64",
      runtimeArch: "x64",
    ...
    expect(
      diagnosticsFromDesktopBridge({
        ...
        diagnostics: {
          ...
          message: "Stream server entrypoint not found",
          processArch: "arm64",
          platform: "darwin",
        },
      }),
